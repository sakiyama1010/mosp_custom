...
public class PortalTimeCardBean extends PortalBean implements PortalBeanInterface {
	...
	public void show() throws MospException {
		...
		if (IpAddressUtility.isAddressAvailable(mospParams) == false) {
			// 始業終業ボタン表示設定(非表示)
			putPortalParameter(PRM_TIME_BUTTON, String.valueOf(ApplicationEntity.CODE_TIME_BUTTON_NONE));
			// ポータル休憩ボタン表示設定(非表示)
			putPortalParameter(PRM_REST_BUTTON, String.valueOf(ApplicationEntity.CODE_REST_BUTTON_NONE));
		}
	}
