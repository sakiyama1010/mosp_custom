...
public class AttendanceCardAction extends TimeAction {
	...
	protected void setRecordStartTime() throws MospException {
		...
		// 【】内に打刻始業時刻を設定
		vo.setLblStartTime(getRecordTimeString(dto.getRecordTime(), dto.getWorkDate()));
	}
	...
	protected void setRecordEndTime() throws MospException {
		...
		// 【】内に打刻終業時刻を設定
		vo.setLblEndTime(getRecordTimeString(dto.getRecordTime(), dto.getWorkDate()));
	}
	...
	protected String getRecordTimeString(Date recordTime, Date workDate) {
		...
		if (recordTime.before(workDate)) {
			sb.append("-");
			workDate = DateUtility.getDate(recordTime);
		}
		sb.append(DateUtility.getStringTimeAndSecond(recordTime, workDate));
	...
	protected void setGoOutDtoFields(List<GoOutDtoInterface> publicList, List<GoOutDtoInterface> privateList,
		...
		// 勤怠設定情報取得
		TimeSettingDtoInterface timeSettingDto = timeReference().cutoffUtil().getTimeSetting(personalId, targetDate);
		// 処理結果確認
		if (mospParams.hasErrorMessage()) {
			return;
		}
