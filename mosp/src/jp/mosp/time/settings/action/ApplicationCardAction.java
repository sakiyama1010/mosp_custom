		...
		if (vo.getRadApplicationType().equals(PlatformConst.APPLICATION_TYPE_MASTER)) {
			...
			for (int i = 0; i < humanList.size(); i++) {
				...
				if (reference().entrance().isEntered(targetId, targetDate) == false) {
					addNotJoinedEmployeesErrorMessage(targetDate, getEmployeeCode(targetId));
					continue;
				}
				// 退社情報の設定
				if (reference().retirement().getRetireInfo(targetId, targetDate) != null) {
					addNotRetirementEmployeesErrorMessage(targetDate, getEmployeeCode(targetId));
					continue;
				}
