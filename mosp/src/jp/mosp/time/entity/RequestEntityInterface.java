...
public interface RequestEntityInterface {
	 ...
	 * 勤務形態を取得する。<br>
	 * <br>
	 * {@link RequestEntityInterface#getWorkType(boolean, boolean) }を参照。<br>
	 * <br>
	 * 承認済フラグはfalseとする。<br>
	 * <br>
	 * @param isAttendanceConsidered 勤怠申請考慮フラグ(true：考慮する、false：しない)
	 ...
	 * @return 振出・休出申請の勤務形態
	 */
	String getWorkOnHolidayWorkType(boolean isCompleted);
	
	/**
	 * 振出・休出申請(休日出勤)が存在するかを確認する。<br>
	 * @param isCompleted 承認済フラグ(true：承認済申請のみ考慮、false：申請済申請を考慮)
	 * @return 確認結果(true：振出・休出申請(休日出勤)が存在する、false：そうでない)
	 */
	boolean isWorkOnHolidaySubstisuteOff(boolean isCompleted);
	 ...
	 */
	Date getWorkOnHolidayEndTime(boolean isCompleted);
	 ...
	 */
	boolean isWorkOnHolidayNotSubstitute(boolean isCompleted);
