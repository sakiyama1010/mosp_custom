...
public interface PaidHolidayDataReferenceBeanInterface extends BaseBeanInterface {
	...
	/**
	 * 期間内に有効な有給休暇情報リストを取得する。<br>
	 * 付与日<期間終了日 かつ 期限日＞期間開始日 の有給休暇情報リストを取得する。
	 * @param startDate 期間開始日
	 * @param endDate 期間終了日
	 * @return 有給休暇データリスト
	 * @throws MospException インスタンスの取得或いはSQL実行に失敗した場合
	 */
	List<PaidHolidayDataDtoInterface> findForAllTermOfEffectiveList(Date startDate, Date endDate) throws MospException;
}
