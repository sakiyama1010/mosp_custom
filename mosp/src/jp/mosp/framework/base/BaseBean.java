...
public abstract class BaseBean implements BaseBeanInterface {
	...
	/**
	 * オブジェクトを生成する。<br>
	 * {@link InstanceFactory#loadInstance(Class, String)}を用いる。<br>
	 * <br>
	 * @param cls        対象オブジェクトインターフェース
	 * @param modelClass モデルクラス名
	 * @return オブジェクト
	 * @throws MospException オブジェクトの生成に失敗した場合
	 */
	protected <T> T createObject(Class<T> cls, String modelClass) throws MospException {
		// インスタンス生成クラスを用いてインスタンスを生成
		return InstanceFactory.loadInstance(cls, modelClass);
	}
