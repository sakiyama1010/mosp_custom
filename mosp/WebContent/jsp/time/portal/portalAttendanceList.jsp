...
<%
for (String[] addonBean : params.getProperties().getCodeArray(TimeConst.CODE_KEY_PORTAL_ATTENDANCE_LIST_ADDONS, false)) {
	String[] portalName = vo.getPortalParameters(addonBean[0]+TimeConst.CODE_KEY_PORTAL_NAME);
	if (portalName.length > 0) {
		
%>
				<th class="ListSelectTh" id="thTime"><%= portalName[0] %></th>
<%
	}
}
%>
...
<%
for(String[] addonBean : params.getProperties().getCodeArray(TimeConst.CODE_KEY_PORTAL_ATTENDANCE_LIST_ADDONS, false)){
	String[] portalClass = vo.getPortalParameters(addonBean[0]+TimeConst.CODE_KEY_PORTAL_CLASS);
	String[] portalValue = vo.getPortalParameters(addonBean[0]+TimeConst.CODE_KEY_PORTAL_VALUE);
	if (portalClass.length > 0 && portalValue.length > 0) {
%>
				<td class="<%= HtmlUtility.escapeHTML(portalClass[i]) %>"><%= HtmlUtility.escapeHTML(portalValue[i]) %></td>
<%
	}
}
%>
