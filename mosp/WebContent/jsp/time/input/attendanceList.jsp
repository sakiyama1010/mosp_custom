...
<% } %>
			...
			<td class="RollTd">
				<%= PlatformNamingUtility.employeeCode(params) %>
			</td>
	...
	<table class="UnderTable" id="list">
		<thead>
			<tr>
				...
				<%
				for(String[] addonBean : params.getProperties().getCodeArray(TimeConst.CODE_KEY_ATTENDANCE_LIST_ADDONS, false)){
					if(vo.getAddonName().get(addonBean[0]) != null){
				%>
				<th class="ListSelectTh" id="thTime"><%= HtmlUtility.escapeHTML(vo.getAddonName().get(addonBean[0])) %></th>
				<%
					}
				}
				%>
...
<% } %>
				...
				<%if(vo.isLblTimesCat()){%>
				<td class="ListSelectTd"><%= HtmlUtility.escapeHTML(vo.getAryLblCat(i)) %></td>
				<%}%>
				...
				<%
				for(String[] addonBean : params.getProperties().getCodeArray(TimeConst.CODE_KEY_ATTENDANCE_LIST_ADDONS, false)){
					if(vo.getAddonValues().get(addonBean[0]) != null){
				%>
				<td class="<%= HtmlUtility.escapeHTML(vo.getAddonStatus().get(addonBean[0])[i]) %>"><%= HtmlUtility.escapeHTML(vo.getAddonValues().get(addonBean[0])[i]) %></td>
				<%
					}
				}
				%>
